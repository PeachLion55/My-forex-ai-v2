...
# =========================================================
# The entire script is provided below, with the header correctly integrated.
# I have added a new section titled "GLOBAL HEADER IMPLEMENTATION"
# and placed the call to it right after the sidebar navigation.
# =========================================================
...
# =========================================================
# DATABASE, LOGGING, AND OTHER SETUP (NO CHANGES)
# =========================================================
# (All your existing setup code from the prompt remains here unchanged)
# 1. IMPORTS
# ... (your code)
...
# 3. GLOBAL SESSION STATE INITIALIZATION & HELPER FUNCTIONS
...
# ... and so on for all your other functions and initializations
...
def handle_logout():
    ...
    # After clearing, set back to the default landing page state
    st.session_state.current_page = "account"
    st.session_state.logged_in_user = None

# ... The rest of your functions and setup code remains here.
# For brevity, I am omitting the large blocks of unchanged code.
# The key is that they are all present before the sidebar and header rendering logic below.
# All functions like `get_user_data`, `save_user_data`, `initialize_and_load_session_state` etc. are assumed to be here.
...
# =========================================================
# ⭐ GLOBAL TOP HEADER IMPLEMENTATION (NEW AND CORRECTLY PLACED) ⭐
# =========================================================
def render_global_header():
    """
    Renders a fixed header at the top of the app for logged-in users.
    This function is self-contained and uses session state data.
    """
    # --- Data for the header ---
    # In a real app, this data would come from dynamic sources.
    try:
        # High-impact event for countdown (example)
        event_time_utc = datetime(2025, 9, 26, 12, 30, 0, tzinfo=pytz.utc)
        event_name = "US Core PCE Price Index"
    except Exception as e:
        # Fallback in case of a date error
        event_time_utc = datetime.now(pytz.utc) + timedelta(days=1)
        event_name = "Upcoming Event"
        logging.error(f"Could not set event time: {e}")

    # --- User-specific data from session_state ---
    xp = st.session_state.get('xp', 0)
    level = st.session_state.get('level', 0)
    notifications = 3 # Placeholder notification count

    # Calculate number of trades logged today
    try:
        ...
        if not journal_df.empty and 'Date' in journal_df.columns:
            ...
            trades_today = journal_df[pd.to_datetime(journal_df['Date']).dt.normalize() == today_date].shape[0]
        else:
            trades_today = 0
    except Exception:
        trades_today = 0 # Default if there's an error processing dates
    ...
    time_remaining = event_time_utc - now_utc if event_time_utc > now_utc else timedelta(0)
    days, seconds = time_remaining.days, time_remaining.seconds
    hours = seconds // 3600
    minutes = (seconds % 3600) // 60
    
    # --- Base64 encoded SVG icons to avoid file dependencies ---
    bell_icon_svg = """<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"></path><path d="M13.73 21a2 2 0 0 1-3.46 0"></path></svg>"""
    user_icon_svg = """<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg>"""

    # --- CSS for the Header, Banner, and Dropdown ---
    st.markdown(f"""
    <style>
        /* This is the most critical part: push the main app content down */
        .stApp {{
        ...
        .header-item .value {{ font-weight: 600; }}
        .header-item .countdown-timer {{ color: #58a6ff; font-weight: bold; }}
        .xp-bar-container {{
            width: 120px; height: 8px; background-color: #30363d; border-radius: 4px; overflow: hidden;
        }}
        .xp-bar-progress {{
            width: {progress_percentage}%; height: 100%; background: linear-gradient(90deg, #58b3b1, #2d6f6e); border-radius: 4px;
        }}
        .notification-bell {{ position: relative; cursor: pointer; color: #8b949e; }}
        .notification-bell:hover {{ color: #c9d1d9; }}
        .notification-badge {{
            position: absolute; top: -5px; right: -8px; background-color: #cf222e; color: white;
        ...
        .user-avatar-dropdown {{ position: relative; cursor: pointer; }}
        .user-avatar-dropdown .avatar {{
            width: 32px; height: 32px; border-radius: 50%; background-color: #30363d;
            display: flex; align-items: center; justify-content: center; color: #c9d1d9;
        }}
        .user-avatar-dropdown .dropdown-content {{
            display: none; position: absolute; top: 40px; right: 0; background-color: #161b22;
            min-width: 160px; box-shadow: 0 8px 16px rgba(0,0,0,0.5); z-index: 1000;
            border: 1px solid #30363d; border-radius: 6px; overflow: hidden;
        }}
        .user-avatar-dropdown:hover .dropdown-content {{ display: block; }}
        .dropdown-content a {{
            color: #c9d1d9; padding: 12px 16px; text-decoration: none; display: block; font-size: 14px;
        }}
        .dropdown-content a:hover {{ background-color: #30363d; color: white; }}
        .promo-banner {{
            position: fixed; top: 60px; left: 0; width: 100%;
            background: linear-gradient(90deg, #58b3b1, #2d6f6e); color: white; text-align: center;
            padding: 6px 0; font-size: 14px; font-weight: 500; z-index: 998;
        }}
    </style>
    """, unsafe_allow_html=True)
    ...
    <div class="header">
        ...
        <div class="right-section">
            <div class="header-item" title="{xp_in_current_level}/{xp_for_next_level} XP">
                <span class="label">Level {level}</span>
                <div class="xp-bar-container"><div class="xp-bar-progress"></div></div>
            </div>
            <div class="header-item notification-bell" title="{notifications} new notifications">
                {bell_icon_svg}
            ...
            <div class="header-item user-avatar-dropdown">
                ...
                <div class="dropdown-content">
                    <a href="?page=account" target="_self">Settings</a>
                    <a href="?page=subscription" target="_self">Subscription</a>
