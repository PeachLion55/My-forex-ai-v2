...
def load_fx_rates():
    ...
    df = pd.DataFrame({
        "symbol": symbols,
        "last": np.random.uniform(1.0, 1.5, size=len(symbols)),
        "chg": np.random.uniform(-0.01,0.01,size=len(symbols)),
        "vol": np.random.randint(1000, 10000, size=len(symbols)),
    })
    return df
...
.stApp {{
    background:
        ...
        radial-gradient(circle at 85% 30%, rgba(173,216,230,{bg_opacity*0.08}) 0%, transparent 30%),
        linear-gradient(180deg, rgba(10,10,25,0.7), rgba(8,8,20,0.6));
    color: #e6eef8;
...
/* Tabs / navigation customizations */
[data-testid="stHeader"] {{
    background: linear-gradient(90deg, rgba(0,0,0,0.2), rgba(0,0,0,0.0));
}}
/* Left navigation */
.left-nav {{
    background: linear-gradient(180deg, rgba(10,10,25,0.85), rgba(8,8,20,0.75));
    padding: 12px;
    border-radius: 10px;
    position: sticky;
    top: 20px;
...
div[data-baseweb="radio"] > label > div {{ padding: 0; }}
div[data-baseweb="radio"] label {{ display: block; }}
/* Improve plotly card background */
.plotly-graph-div {{
    background: transparent !important;
}}
/* Enhanced tab styling for sub-tabs */
...
with header_col1:
    st.markdown(
        ...
        '<img src="https://img.icons8.com/ios-filled/50/ffffff/line-chart.png" style="height:38px;">'
        '<div style="line-height:1">'
        '<div style="font-size:18px;font-weight:700;color:#fff;">Forex Pro Dashboard</div>'
        '<div style="font-size:12px;color:#aab6d6;">Real-time rates · backtesting · trade ideas</div>'
        '</div></div>',
        unsafe_allow_html=True
    )
with header_col2:
    st.markdown(
        "<div style='text-align:right; font-size:12px; color:#9aa7c7'>UTC: "
        + dt.datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S") + "</div>",
        unsafe_allow_html=True
    )
...
col_nav, col_main = st.columns([1, 5], gap="small")
with col_nav:
    st.markdown(
        """<style>
        .left-nav { background: linear-gradient(180deg, rgba(10,10,25,0.85), rgba(8,8,20,0.75));
            padding: 12px; border-radius: 10px; position: sticky; top: 20px; height: calc(100vh - 140px); overflow: auto;}
        div[data-baseweb="radio"] > label > div { padding: 0;}
        div[data-baseweb="radio"] label { display:block;}
        </style>""",
        unsafe_allow_html=True
    )
    page = st.radio("", nav_options, index=0, key="main_nav", label_visibility="collapsed")
...
with col_main:
    if page == "Forex Fundamentals":
        ...
        # Simple equity curve
        eq = np.cumsum(np.random.normal(0, 1, 200))
        ...
        # Sample trades table
        trades = pd.DataFrame({
            "time": pd.date_range(dt.datetime.utcnow()-dt.timedelta(days=30), periods=10, freq="12H"),
            "symbol": np.random.choice(["EURUSD","GBPUSD"], 10),
            "pnl": np.random.normal(0, 50, 10)
        })
