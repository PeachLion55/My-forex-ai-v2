...
/* Button styling (smaller size, smaller font, 90% opacity gradient) */
.stButton button {{
...
/* Form submit button styling (same as other buttons, 90% opacity gradient) */
...
import math, uuid, glob, datetime as dt
import numpy as np
import plotly.express as px
import plotly.graph_objects as go
...
def _ta_show_badges(df):
    # Lightweight inline gamification widget
    try:
...
with selected_tab[6]:
    st.title("🧠 Psychology")
    df = globals().get("trades_df") or globals().get("journal_df")
    if df is None or len(df)==0 or "emotions" not in df.columns:
...
# --- MT5 Stats Dashboard: Edge Finder & Customisable Dashboard ---
with selected_tab[4]:
    try:
        df = globals().get("trades_df") or globals().get("journal_df")
    except Exception:
    ...
    if df is None or df is ... or df is False or getattr(df, 'empty', False):
